        ...
        <form action="{{ route('admin.jadwal.update', $jadwal) }}" method="POST" class="p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="date" id="tanggal_praktek" name="tanggal_praktek" value="{{ old('tanggal_praktek', $jadwal->tanggal_praktek->format('Y-m-d')) }}" required
                        class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                ...
                <div>
                    ...
                    <input type="number" id="kuota" name="kuota" value="{{ old('kuota', $jadwal->kuota) }}" min="1" max="100" required
                        class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                    @error('kuota')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="md:col-span-2 flex items-center">
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $jadwal->is_active) ? 'checked' : '' }} class="sr-only peer">
