<x-admin-layout>
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-bold text-gray-800 dark:text-white mt-4">Edit Dokter</h1>
    </div>

    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden">
        <form action="{{ route('admin.dokter.update', $dokter) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" id="kode_dokter" name="kode_dokter" value="{{ old('kode_dokter', $dokter->kode_dokter) }}" required
                ...
                <div>
                    ...
                    <input type="text" id="rumah_sakit" name="rumah_sakit" value="{{ old('rumah_sakit', $dokter->rumah_sakit) }}"
                        class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                    @error('rumah_sakit')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="url" id="foto" name="foto" value="{{ old('foto', $dokter->foto) }}"
                        class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                    @error('foto')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="flex items-center">
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="konsultasi_online" value="1" {{ old('konsultasi_online', $dokter->konsultasi_online) ? 'checked' : '' }} class="sr-only peer">
